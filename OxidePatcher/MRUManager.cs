...
namespace OxidePatcher
{
    class MRUManager
    {
        ...
        /// </summary>
        public Action<object, EventArgs> OnRecentFileClick { get; private set; }
        ...
        private void Refresh()
        {
            ...
            foreach (var file in recentFiles.Select(entry => (string)registryKey.GetValue(entry)).Where(file => file != null))
            {
            ...
            toolStripItem.Click += new EventHandler(Clear);
        ...
        public void AddOrUpdate(string filename)
        {
            try
            {
                var registryKey = Registry.CurrentUser.CreateSubKey(SubKeyName,
                    RegistryKeyPermissionCheck.ReadWriteSubTree);
                if (registryKey == null) return;
                var recentFiles = registryKey.GetValueNames();
                var newRecentFiles = new List<string> { filename };
                ...
                for (var i = 0; i < newRecentFiles.Count; i++)
                {
        ...
        public void Remove(string filename)
        {
            try
            {
                var registryKey = Registry.CurrentUser.OpenSubKey(SubKeyName, true);
                if (registryKey == null) return;
                var recentFiles = registryKey.GetValueNames();
                foreach (var file in recentFiles.Where(file => (string) registryKey.GetValue(file, null) == filename))
                {
                    registryKey.DeleteValue(file, true);
                    registryKey.Close();
                    break;
